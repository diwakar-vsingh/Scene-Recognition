...
def create_datasets(data_path, input_size, rgb=False):
  ...
  details. 
  Although you don't need to for this project, you can also create your
  ...
  train_data_tforms.append(transforms.ColorJitter())
  ...
  train_dset= datasets.ImageFolder(root=osp.join(data_path, 'train'), transform=train_data_tforms)
  test_dset = datasets.ImageFolder(root=osp.join(data_path, 'test'), transform=test_data_tforms)
...
class SimpleNet(nn.Module):
  ...
  def __init__(self, num_classes, droprate=0.5, rgb=False, verbose=False):
    ...
    self.features = nn.Sequential(
      nn.Conv2d(in_channels=in_channels, out_channels=10, kernel_size=5, stride=1, padding=2, bias=False),
    ...
    self.classifier = nn.Sequential(
      nn.Conv2d(in_channels=32, out_channels=num_classes, kernel_size=8, stride=1, padding=0, bias=False)
    )
    ...
    for name, m in self.named_modules():
      if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
        # Initializing weights with randomly sampled numbers from a normal distribution.
        nn.init.kaiming_normal_(m.weight.data, a=0, mode='fan_in', nonlinearity='relu')
        if m.bias is not None:
          nn.init.constant_(m.bias.data, 0)
      elif isinstance(m, nn.BatchNorm2d):
        # Initialize the weights and biases for BatchNorm
...
def custom_part1_trainer(model):
    ...
    'optimizer': optim.Adam(params=model.parameters(), lr=1e-3, betas=(0.9, 0.999), eps=1e-08, weight_decay=0),
    'params': {'n_epochs': 100, 'batch_size': 50, 'experiment': 'part1'}
    }

    part1['lr_scheduler'] = optim.lr_scheduler.StepLR(part1['optimizer'], step_size=60, gamma=0.1)
    # part1['lr_scheduler'] = optim.lr_scheduler.ExponentialLR(part1['optimizer'], gamma=0.98)
    return part1
...
def create_part2_model(model, num_classes):
  ...
  second_last_layer = nn.Dropout(p=0.5)
  new_classifier.append(second_last_layer)
  ...
  nn.init.kaiming_normal_(last_layer.weight.data, a=0, mode='fan_in', nonlinearity='relu')
...
def custom_part2_trainer(model):
    ...
    part2 = {
    'loss_function': nn.CrossEntropyLoss(),
    'optimizer': optim.Adam(params=params_to_optimize, lr=1e-3, betas=(0.9, 0.999), eps=1e-08, weight_decay=0),
    'params': {'n_epochs': 4, 'batch_size': 10, 'experiment': 'part2'} 
    }

    part2['lr_scheduler'] = optim.lr_scheduler.StepLR(part2['optimizer'], step_size=10, gamma=0.1)
    return part2
